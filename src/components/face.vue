  ...
  <div style="height:100%">
    <div class="face">
      <p class="step-title">
            ...
            <!-- <img class="step-titleimg" @click="videoImg=!videoImg" :src="videoImg ? '/static/images/landpage/liveface/open.png' : '/static/images/landpage/liveface/close.png'" alt="" /> -->
         </p>
...
export default {
  ...
  mounted() {
    ...
    setInterval(()=>{getFACE()},5000);

    this.$nextTick(()=>{  //音频控制设置
  ...
  methods: {
    ...
    getMediaBegin() {
      ...
      if (navigator.mediaDevices.getUserMedia) {
        //最新的标准API
        navigator.mediaDevices.getUserMedia(_this.constraints).then(_this.getMediaSuccess).catch(_this.getMediaError);
      } else if (navigator.webkitGetUserMedia) {
        //webkit核心浏览器
        navigator.webkitGetUserMedia(_this.constraints,_this.getMediaSuccess, _this.getMediaError)
      } else if (navigator.mozGetUserMedia) {
        //firfox浏览器
        navigator.mozGetUserMedia(_this.constraints, _this.getMediaSuccess, _this.getMediaError);
      } else if (navigator.getUserMedia) {
        //旧版API
        navigator.getUserMedia(_this.constraints, _this.getMediaSuccess, _this.getMediaError);
      } else {
        ...
        if (ua.match(/iPhone\sOS/i) == "iphone os") {
          ...
          _this.$Message.error('您的设备没有安装摄像头，或者不支持视频录入，请更换设备或者更换浏览器再访问');
        }
      }
    },
