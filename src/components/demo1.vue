...
export default {
  ...
  methods: {
    ...
    keep() {
      ...
      this.resultLitigantData.signPath = this.url; //签名路径
    ...
    clickFinish() {
      ...
      this.$store.commit("nameDirection", this.direction);
      this.namePath = this.$store.getters.namePath
      this.nameDirection = this.$store.getters.nameDirection 
      this.handDraw = false;
      this.myBookShow = true;

      alert(this.nameDirection)
      // this.clickReset();
    },
    clickClose() {
      this.handDraw = false;
